...
test.describe('Wishlist Functionality', () => {
  ...
  test('should remove products from wishlist', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="wishlist-count"]')).toHaveText('0')
  })
  ...
  test('should open wishlist sidebar', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="wishlist-sidebar"]')).toBeVisible()
  ...
  test('should clear entire wishlist', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      const wishlistButton = page.locator('.card').nth(i).locator('button.btn-circle')
      await wishlistButton.click()
    }
  ...
  test('should persist wishlist state across page navigation', async ({ page }) => {
    // Add product to wishlist
    const wishlistButton = page.locator('.card').first().locator('button.btn-circle')
    await wishlistButton.click()
    
    // Get wishlist count
    const wishlistCount = await page.locator('[data-testid="wishlist-count"]').textContent()
    ...
    await expect(page.locator('[data-testid="wishlist-count"]')).toHaveText(wishlistCount || '0')
    
    // Product should still be in wishlist (button should be primary)
    const newWishlistButton = page.locator('.card').first().locator('button.btn-circle')
  ...
  test('should close wishlist sidebar', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="wishlist-sidebar"]')).not.toBeVisible()
  })
  ...
  test('should handle multiple wishlist operations', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="wishlist-count"]')).toHaveText('3')
    
    // Remove one product
    const firstWishlistButton = page.locator('.card').first().locator('button.btn-circle')
    ...
    const fourthWishlistButton = page.locator('.card').nth(3).locator('button.btn-circle')
