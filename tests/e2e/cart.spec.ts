...
test.describe('Cart Functionality', () => {
  ...
  test('should add products to cart', async ({ page }) => {
    ...
    const finalCartCount = await page.locator('[data-testid="cart-count"]').textContent()
    expect(parseInt(finalCartCount || '0')).toBe(parseInt(initialCartCount) + 2)
  })
  ...
  test('should update product quantity in cart', async ({ page }) => {
    ...
    const initialQuantity = await page.locator('[data-testid="quantity-input"]').inputValue()
    
    // Increase quantity
    await page.click('[data-testid="quantity-increase"]')
    
    // Quantity should increase
    const newQuantity = await page.locator('[data-testid="quantity-input"]').inputValue()
    ...
    const finalQuantity = await page.locator('[data-testid="quantity-input"]').inputValue()
  ...
  test('should calculate cart total correctly', async ({ page }) => {
    ...
    const firstProductPrice = await page.locator('[data-testid="cart-item-price"]').textContent()
    const firstPrice = parseFloat(firstProductPrice?.replace(/[^0-9.]/g, '') || '0')
    ...
    const secondProductPrice = await page.locator('[data-testid="cart-item-price"]').nth(1).textContent()
    const secondPrice = parseFloat(secondProductPrice?.replace(/[^0-9.]/g, '') || '0')
  ...
  test('should clear entire cart', async ({ page }) => {
    ...
    await page.click('.card button:has-text("Add to Cart")')
    await page.locator('.card').nth(1).locator('button:has-text("Add to Cart")').click()
    await page.locator('.card').nth(2).locator('button:has-text("Add to Cart")').click()
  ...
  test('should persist cart state across page navigation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="cart-count"]')).toHaveText(cartCount || '0')
  })

  test('should handle cart with many items', async ({ page }) => {
    // Add multiple products to cart
    for (let i = 0; i < 5; i++) {
      await page.locator('.card').nth(i).locator('button:has-text("Add to Cart")').click()
    }
