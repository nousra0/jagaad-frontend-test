...
export class TestHelpers {
  ...
  async openCart() {
    ...
    await expect(
      this.page.locator('[data-testid="cart-sidebar"]')
    ).toBeVisible();
  }
  ...
  async openWishlist() {
    ...
    await expect(
      this.page.locator('[data-testid="wishlist-sidebar"]')
    ).toBeVisible();
  }
   ...
   */
  async sortProducts(
    option: 'default' | 'price-high-low' | 'price-low-high' | 'alphabetical'
  ) {
  ...
  async removeProductFromWishlist(index: number = 0) {
    ...
    await this.page
      .locator('[data-testid="remove-wishlist-item"]')
      .nth(index)
      .click();
  }
