    ...
    <div class="drawer-side">
      ...
      <div class="min-h-full p-4 w-80 bg-base-200 text-base-content">
        <div class="flex justify-between items-center mb-6">
          ...
          <label for="cart-drawer" class="btn btn-sm btn-circle btn-ghost">✕</label>
        </div>
        ...
        <div v-else class="space-y-4">
          <div v-for="item in cartStore.items" :key="item.product.title" class="card bg-base-100 shadow-sm">
            <div class="card-body p-4">
              <div class="flex items-start space-x-3">
                ...
                <div class="flex-1 min-w-0">
                  <h3 class="font-semibold text-sm line-clamp-2">{{ item.product.title }}</h3>
                  <p class="text-primary font-bold text-sm">
                    {{ formatCurrencyWithCode(item.product.price, item.product.currency) }}
                  </p>
                </div>
                <button 
              ...
              <div class="flex justify-between items-center mt-3">
                ...
                <div class="text-sm font-semibold">
                  {{ formatCurrencyWithCode((parseFloat(item.product.price) * item.quantity).toString(), item.product.currency) }}
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card bg-base-100 shadow-sm">
            <div class="card-body p-4">
              <div class="flex justify-between items-center text-lg font-bold">
                <span>Total:</span>
                <span class="text-primary">{{ formatCurrencyWithCode(cartStore.cartTotal.toString(), 'EUR') }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-2">
            ...
            <button @click="cartStore.clearCart()" class="btn btn-outline btn-error w-full">
              Clear Cart
            </button>
          </div>
        </div>
